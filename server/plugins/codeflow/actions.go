...
func NewRelease(f Feature, u User, release *Release) error {
	...
	results := db.Collection("secrets").Find(bson.M{"global": true, "type": bson.M{"$in": []plugins.Type{plugins.Env, plugins.ProtectedEnv, plugins.File}}, "deleted": false})
	for results.Next(&secret) {
		secrets = append(secrets, secret)
	}

	results = db.Collection("secrets").Find(bson.M{"projectId": f.ProjectId, "type": bson.M{"$in": []plugins.Type{plugins.Env, plugins.ProtectedEnv, plugins.File}}, "deleted": false})
	for results.Next(&secret) {
		secrets = append(secrets, secret)
	}
